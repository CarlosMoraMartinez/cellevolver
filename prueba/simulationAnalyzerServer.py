...
class simulationHolder(object):
	...
	def __init__(self, name, path):
		...
		#self.type4 = self.__setType4() not yet
		self.motifList = None
	def __setType2(self):
		t = cp.deepcopy(self.expr.type)
	...
	def makeHeatmaps(self):
		...
		for muttype in ['tfs', 'sites']:
			...
			for i in np.unique(self.simulations[0].type3):
				...
				xaux, taux = self.joinMutEffects(type1 = None, type2=None, type3=[i], regType1=['1', '-1'], regType2=None, regType3=None, sortByCell=False, regList=None, mutation_type = muttype, difference_to_wt=True, error=True, type_used_sort = 3, sort_gene_in_each_cell=True)
				...
				x3e = np.concatenate((np.sort(x2[:,:-1], axis=1)[:, ::-1], xaux), axis=1)
...
def main():
	...
	#paths= ['../cellevolver_shared/' + i for i in conditions]
