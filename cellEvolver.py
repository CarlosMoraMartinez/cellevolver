...
class multicellEvolver():
	...
	def tfsToDF(self, outname):
		...
		for i in range(self.template.tf_num):
			aux = pd.DataFrame()
			for l in m[i].pos.alphabet: #.letters in previous version, now it's a tuple
				aux[l] = m[i].pos[l]  #.get(l)
		...
		self.interactionsToDF(outname) #return of this function not used
		return df
