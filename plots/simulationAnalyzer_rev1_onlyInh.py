...
class simulationSet(object):
	...
	def makeHeatmaps(self):
		...
		g0.data.to_csv(self.path + '_activatorExpression.csv')
		g0oth.savefig(self.path + '_activatorOthExpression.png')
		g0oth.data.to_csv(self.path + '_activatorOthExpression.csv')
		g0lin.savefig(self.path + '_activatorLinExpression.png')
		g0lin.data.to_csv(self.path + '_activatorLinExpression.csv')
		g1.savefig(self.path + '_inhibitorExpression.png')
		g1.data.to_csv(self.path + '_inhibitorExpression.csv')
		g2.savefig(self.path + '_allTFExpression.png')
		g2.data.to_csv(self.path + '_allTFExpression.csv')
		plt.close("all")
		## TFS /sites
		## TFS /sites
		x3={}
		for muttype in ['tfs', 'sites']:
			...
			for i in np.unique(self.simulations[0].type3):
				...
				gaux = sns.clustermap(xaux, col_cluster=False,  col_colors=colcol3,row_colors=rowcol3, method='ward', center = 0, vmin = -3.5, vmax = 2)
				...
				gaux.data.to_csv(self.path + '_' + muttype + 'Phenotype_' + i + '.csv')
				...
				x3e = np.concatenate((np.sort(x1[:,:-1], axis=1)[:, ::-1], xaux), axis=1)
				gauxe = sns.clustermap(x3e, col_cluster=False,  col_colors=colcol3e,row_colors=rowcol3, method='ward', center = 0, vmin = -3, vmax = 2)
				...
				gauxe.data.to_csv(self.path + '_' + muttype + 'Phenotype_' + i + '_andEXPR.csv')
				...
				gauxe = sns.clustermap(x4e, col_cluster=False,  col_colors=colcol4e,row_colors=rowcol3, method='ward', center = 0, vmin = -3, vmax = 2)
				gauxe.ax_heatmap.set_xticklabels([''])
				gauxe.ax_heatmap.set_yticklabels([''])
				gauxe.ax_heatmap.set_xlabel(muttype + 'effect in '+ i +' per cell', fontsize = 15)
				gauxe.ax_heatmap.set_ylabel('TF', fontsize = 15)
				gauxe.savefig(self.path + '_' + muttype + 'Phenotype_' + i + '_andEXPR_cell0.png')
				gauxe.data.to_csv(self.path + '_' + muttype + 'Phenotype_' + i + '_andEXPR_cell0.csv')
				plt.close("all")
			## combinations		
			tojoin = [j for j in np.unique(self.simulations[0].type3) if 'lin' not in j]
			#x5 = np.sort(x2[:,:-1], axis=1)[:, ::-1] # with inhibitors
			x5 = np.sort(x1[:,:-1], axis=1)[:, ::-1]
			...
			g5.data.to_csv(self.path + '_' + muttype + 'Phenotype_TerminalJoinAndExpr.csv')
			...
			g6.data.to_csv(self.path + '_' + muttype + 'Phenotype_TerminalJoinAndExpr_cell0.csv')
...
def main():
	...
	paths= ['/home/carmoma/projects/cellevolver/data/all_tables/' + i for i in conditions]
