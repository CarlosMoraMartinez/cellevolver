	...
	def makeHeatmaps(self):
		...
		for muttype in ['tfs', 'sites']:
			...
			for i in np.unique(self.simulations[0].type3):
				...
				gaux = sns.clustermap(xaux, col_cluster=False,  col_colors=colcol3,row_colors=rowcol3, method='ward', center = 0, vmin = -3, vmax = 2)
			...
			g5 = sns.clustermap(x5, col_cluster=False,  col_colors=colcol5,row_colors=rowcol5, method='ward', center = 0, vmin = -3, vmax = 2)
			...
			# combinations only in cell 3
			colcol6 = []
			for lev in range(len(colcol5)):
				colcol6.append([n for k, n in enumerate(colcol5[lev]) if colcol5[0][k] == cellmap[3] or colcol5[2][k] == 'w'])
			x6 = x5[:, [k for k in range(len(colcol5[1])) if cellmap[3] == colcol5[0][k] or colcol5[2][k]== 'w']]
			g6 = sns.clustermap(x6, col_cluster=False,  col_colors=colcol6,row_colors=rowcol5, method='ward', center = 0, vmin = -3, vmax = 2)
