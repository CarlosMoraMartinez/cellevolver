...
processSim <- function(sim, exprf, tff, sitesf, calc_mode = c("diff", "proportion")){
  ...
  phh <- getPhenotype(expr, tf, sites, sim, calc_mode)
  return(phh)
}
...
getType2<-function(wt){
  res<-sapply(1:nrow(wt), FUN=function(i){
    cell <- as.numeric(gsub("exp", "", wt[i, "cell"]))
    if(!grepl("\\[", wt[i, "type"]) & nchar(wt[i, "type"])==length(unique(wt$cell))){
      lin <- which(strsplit(wt[i, "type"], "")[[1]] != "0") - 1
      if(length(lin)>1){
        return("lineage all")
      }else if(cell == lin){
        return("lineage this")
...
getPhenotype<- function(expr, tf, sites, simname="", calc_mode = c("diff", "proportion") ){
  wt <- melt(expr, id.vars=c("gene", "type"), measure.vars = names(expr)[grep("exp", names(expr))], variable_name = "cell")
  ...
  if(all(tf2$tf_mutated == s2$mutated_sites) & all(tf2$gene == s2$gene)){
    both <- data.frame(gene=tf2$gene, mutation=tf2$tf_mutated,cell=tf2$cell, tf_mutated=tf2$value, mutated_sites=s2$value, stringsAsFactors=FALSE)
...
histogramBase<-function(mu){
  g1 <- ggplot(data = mu, aes(x = regulators, y=percent, fill=type2, group=type2, linetype=type2)) + 
    ...
    geom_bar(stat = "identity", position=position_dodge2(width = 0.9, preserve = "single")) + #position="dodge"
...
newdir <-"./190109phenotypes_by_gene"
...
for(cond in condlist){
  ...
  makeRegnumHistograms(df2, cond, TRUE)
  cat("\n******************************\n", cond, "\n*****************************\n")
}
