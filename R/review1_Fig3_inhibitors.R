...
setwd("/home/carmoma/projects/cellevolver/200816_PLOSoNE_review1_making/200916_data_heatmaps_repressors/")
...
readHeatmapFile <- function(file){
  ...
  d$phenotype_group <- sapply(d$phenotype, function(x){
    ...
    factor(levels=c("<= 0.5", "[0, -0.5[", "[0.5, 0[", "[1, 0.5[", "[2, 1[", "> 2")) #"[-3, -2[" #c("< -2", "[-2, -1[", "[-1, -0.5[", "[-0.5, 0]", "> 0")
  ...
  vars2plot <- names(d)[grep("thresh_", names(d))]
   for(var in vars2plot){
    ...
    d2 <- d2 %>% mutate(expr_group = factor(expr_group, 
      ...
      filter(grepl("other", expr_group) | grepl("terminal all", expr_group))
